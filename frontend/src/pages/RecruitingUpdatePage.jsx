...
import { fetchDynasties, analyzeRecruiting, commitRecruiting } from "../lib/apiClient";
...
function TeamRow({ row, colleges, onChange }) {
  ...
  return (
    <tr className="border-b border-border align-top dark:border-darkborder">
      ...
      <td className="p-2">
        <NumberInput value={row.ranking} onChange={(ranking) => update({ ranking })} />
      </td>
      <td className="p-2">
        <NumberInput value={row.totalSigned} onChange={(totalSigned) => update({ totalSigned })} />
      </td>
      <td className="p-2">
        <NumberInput value={row.nilSpent} onChange={(nilSpent) => update({ nilSpent })} />
      </td>
      <td className="p-2">
        <NumberInput value={row.fiveStars} onChange={(fiveStars) => update({ fiveStars })} />
      </td>
      <td className="p-2">
        <NumberInput value={row.fourStars} onChange={(fourStars) => update({ fourStars })} />
      </td>
      <td className="p-2">
        <NumberInput value={row.threeStars} onChange={(threeStars) => update({ threeStars })} />
      </td>
      <td className="p-2">
        <NumberInput value={row.twoStars} onChange={(twoStars) => update({ twoStars })} />
      </td>
      <td className="p-2">
        <NumberInput value={row.oneStars} onChange={(oneStars) => update({ oneStars })} />
      </td>
      <td className="p-2">
        <NumberInput value={row.points} onChange={(points) => update({ points })} />
      </td>
      <td className="p-2">
...
function RecruitingReview({ rows, colleges, onChange, onCommit, committing, error }) {
  ...
  return (
    <Card>
      <div className="p-5 space-y-4">
        <h3 className="font-varsity text-lg uppercase tracking-[0.06em] text-charcoal dark:text-white">Review Recruiting Classes</h3>
          ...
          <table className="w-full min-w-[1200px] border-collapse text-left">
            <thead>
              <tr className="border-b border-border text-xs uppercase tracking-wide text-textSecondary dark:border-darkborder">
                ...
                <th className="p-2">5-Star</th>
                <th className="p-2">4-Star</th>
                <th className="p-2">3-Star</th>
                <th className="p-2">2-Star</th>
                <th className="p-2">1-Star</th>
        ...
        >
          {committing ? "Saving..." : "Save Recruiting"}
        </button>
      </div>
    </Card>
  );
...
function RecruitingUpdatePage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const result = await analyzeRecruiting(dynastyId, seasonId, files);
      setRows(result.rows);
  ...
  const handleCommit = async () => {
    ...
    setCommitError(null);
    try {
      const result = await commitRecruiting(dynastyId, seasonId, rows);
      ...
      {saved ? (
        <Card>
          <div className="p-5 space-y-2">
            <p className="text-sm font-semibold text-success">Saved! Recruiting classes have been updated.</p>
            {warnings.length > 0 && (
        ...
        <Card>
          <div className="p-5 space-y-4">
            ...
            <p className="text-sm text-textSecondary">
              Upload the national recruiting class rankings screenshot(s). This applies to your current season —
              the AI reads every team&rsquo;s rank, total signed, NIL total, star breakdown, and points and
              proposes the table below for you to review before anything is saved.
            </p>

            <FileDropZone
              title="Recruiting Rankings Screenshots"
              hint="Upload as many screenshots as it takes to cover the whole list."
              files={files}
...
export default RecruitingUpdatePage;
